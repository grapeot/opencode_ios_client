...
- AI 执行时：send 按钮**始终保留**（这是产品的既定行为——运行中也能直接发新消息，不必先终止），stop 作为一个**额外**的按钮出现在 send **下方**（同尺寸的实底圆角方块，VStack 上下排列，send 在上 / stop 在下），AI 结束后消失。注意 stop ≠ 替换 send；二者并存、上下堆叠。配色上 send 用 `accent` 蓝、stop 用红（破坏性/中断语义）。
...
# Pixelate 升级（探索稿 — 仅设计，未实现）

这一节是在上面 Quiet Tech 语言之上叠加的一层**像素化探索**，灵感来自 VoiceFlow 那次 Pixelate 改造（OP-1 / Playdate / 《Obra Dinn》那种"像素即纪律"的高级路线，不是廉价 8-bit 怀旧）。**本节只锁定设计方向 + 效果图，不改代码**；落地放到后续 PR。

和 VoiceFlow 的关键区别：OpenCode 是冷电蓝的开发者工具，可读性优先，所以**像素只作为克制的点缀（garnish），不做全像素**。正文、工具卡内容、代码预览全部保持现代可读 sans-serif / mono，不像素化。像素只出现在三处：小图标、logo、以及"AI 工作中"指示器。

## 一、工具卡渲染：按工具类型分两种形态

这是本次升级最大的改动，也是和现状差异最大的地方。现状是所有工具一律走"通用展开式 DisclosureGroup + wrench 图标"，patch 又被单独做成整卡蓝的导航卡——结果工具卡彼此没有语义区分，patch 概念还被拆成两种卡。

新设计按**工具是否在操作一个具体文件**分两类：

**形态 A — 文件卡（patch / edit / write / read 四种）。** 这四种工具的共同点是"针对一个文件"，所以渲染成一张**看起来就是文件**的卡：左侧一个电蓝**像素文件图标**，中间该文件的 monospace 路径（如 `src/api/client.ts`），右侧一个电蓝像素 chevron 可跳转/展开。一个工具一张文件卡，多个文件就是多张堆叠。判断依据是 `part.tool ∈ {apply_patch, edit_file, write_file, read_file}`（含历史别名）。

**形态 B — 合并可展开单行（其余所有工具：bash / 运行测试 / grep / glob / list / webfetch / task …）。** 这些工具不针对单一文件，把它们**合并成一行摘要**，点 chevron 展开看具体内容。摘要文案按工具语义动态生成，例如：`▸ Ran 3 tests`、`▸ Ran 2 commands`、`▸ Searched 4 files`。同类相邻的多次调用应聚合成一行（"Ran 3 tests" 而不是三行 "bash"）。收起是默认态，展开看每条具体 input/output。

两形态都保持 Quiet Tech 的中性卡身（neutral `surface` 底、无描边、12pt 圆角），彩色只落在像素图标和 chevron 上——全屏至多一处蓝。

![工具卡两形态：文件卡 + 合并单行](design_images/pixelate_tool_cards.png)

> 注意这张 mockup 里：上半"Files updated"是三张文件卡（`client.ts` / `types.ts` / `README.md`），下半"Actions"是三条合并单行（`Ran 3 tests` / `Ran 2 commands` / `Searched 4 files`）。正文是可读 sans-serif，像素只在图标和 chevron。右上角**没有用户头像**（不要 JD 那种），模型选择器保持现状。

## 二、"AI 工作中"像素指示器

现状用一个 gold 30% 透明的缩小 ProgressView。新设计把它换成一个**像素动画指示器**：一小排 chunky 蓝色（或金色，沿用"动态/工作中"语义）像素方块，像一条像素进度条左右流动。它正好对应本设计语言原则 2/3——"金色只给 AI 工作中"那条瞬时态规则用像素方块来表达，既统一又有个性，而且不碰正文。放在当前活动消息附近。

这两张早期探索图里能看到这个 AI-working 像素指示器和整体气质（iOS / Android 各一）：

![Pixelate 探索 — iOS](design_images/pixelate_explore_ios.png)

![Pixelate 探索 — Android](design_images/pixelate_explore_android.png)

## 三、Logo / App Icon 像素化

把 logo 换成像素风，和这套语言一致：一个电蓝**像素终端提示符**——chunky 的 `>` chevron + 闪烁光标块，一眼"编码工具"，冷电蓝单色、近黑底。取代当前那个蓝底方块 icon。app 内 logo（空 session 态的品牌视觉）和 AppIcon 都换。

![OpenCode 像素 logo](design_images/pixelate_logo.png)

## 不做 / 边界

- **不全像素化**：正文、工具卡内容、代码预览、模型选择器保持现代可读字体。像素只在图标 / logo / AI-working 指示器。
- **不要用户头像**：右上角不放 JD 那种用户名/头像。
- **模型选择器不动**：保持现状。
- **本节不改代码**：这是设计探索稿；ToolPartView / PatchPartView 的重构、AI-working 指示器、logo 替换都放后续实现 PR。
