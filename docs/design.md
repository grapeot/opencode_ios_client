...
**不是像素风。** 这一节就在上面 Quiet Tech 语言内，不引入任何新的视觉质感——不要像素图标、不要像素字、不要像素 logo。保持现有的干净现代图标、细分隔、中性卡身。这次真正要改的是**工具结果的信息组织方式**，不是它的皮肤。

现状问题：所有工具一律走"通用展开式 DisclosureGroup + wrench 图标"，patch 又被单独做成整卡蓝的导航卡——工具卡彼此没有语义区分，读起来是一堆同质的灰卡。

## 工具卡按类型分两种形态，按时间顺序混排

按**工具是否在操作一个具体文件**分两类形态。关键：**不分组、不加 "Files updated" / "Actions" 之类的分区标题，就按工具实际发生的时间顺序混排成一条连续的时间线**——该是文件卡的渲染成文件卡，该是单行的渲染成单行，谁先发生谁在上。

**形态 A — 文件卡（patch / edit / write / read 四种）。** 这四种工具的共同点是"针对一个文件"，渲染成一张**看起来就是文件**的卡：左侧一个干净的**文件图标**（保留，普通现代图标即可，不是像素），中间该文件的 monospace 路径（如 `src/api/client.ts`），右侧一个 chevron 可跳转/展开。一个工具一张文件卡。判断依据是 `part.tool ∈ {apply_patch, edit_file, write_file, read_file}`（含历史别名）。

**形态 B — 合并可展开单行（其余所有工具：bash / 运行测试 / grep / glob / list / webfetch / task …）。** 这些工具不针对单一文件，**合并成一行摘要**，点 chevron 展开看具体内容。摘要文案按工具语义动态生成：`▸ Ran 3 tests`、`▸ Ran 2 commands`、`▸ Searched 4 files`。同类相邻的多次调用聚合成一行（"Ran 3 tests" 而不是三行 "bash"）。收起是默认态。

两形态都用现有 Quiet Tech 的中性卡身（neutral `surface` 底、无描边、12pt 圆角），彩色只落在文件图标和 chevron 上——全屏至多一处蓝。

![工具卡两形态，按时间顺序混排](design_images/tool_cards_chronological.png)

> 这张 mockup 是最终方向：文件卡（`client.ts` / `types.ts` / `README.md`，各带一个 file 图标）和单行摘要（`Ran 3 tests` / `Searched 4 files` / `Ran 2 commands`）**按时间顺序交替排列，没有分组标题**。正文是干净 sans-serif（非像素）。AI 回复**左边没有头像 / 圆形图标**——直接是文字；用户消息用一条蓝色左竖条与 AI 回复区分（沿用现有"左色条"语言）。

## 配套的小改动

- **AI 回复不要头像**：AI 消息左边不放圆形 logo / avatar，直接是正文。用户消息 vs AI 回复的区分继续靠"用户消息蓝色左竖条 / AI 回复无容器"这套既有语言。
- **"AI 工作中"指示器**：维持现有"动态/金色只在工作中"的语义即可（gold ProgressView 或等价的轻动效），不引入像素。

## 不做 / 边界

- **不引入像素风**：之前吸引人的是"文件卡 / 合并单行 / 时间序"这套**信息组织**，不是像素质感。整体保持干净克制的 Quiet Tech。
- **不分组、不加分区标题**：工具结果按时间顺序排，不按类型聚类成 "Files updated" / "Actions" 区。
- **文件卡保留 file 图标**；AI 回复不要头像图标。
- **模型选择器不动，不要右上角用户头像。**
- **本节不改代码**：这是设计稿；`ToolPartView` / `PatchPartView` 的重构（两形态 + 时间序混排）、去 AI 头像，都放后续实现 PR。
