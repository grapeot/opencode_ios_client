...
- text review field：语音转写或 retry 期间的 partial transcript 会持续追加/替换输入内容，文本框应自动滚到末尾，像打字机一样露出最新文字；手动编辑和草稿恢复保持系统默认滚动行为，避免抢用户光标。
...
- 语音输入：mic 始终在左侧竖排的**底部槽位**。录音中只显示红色 mic，点击它走正常结束录音并进入转写；转写中 mic 转圈，右侧显示强制 abort 的 `Stop transcription wait`；显式 abort 后左侧底部槽位变为 retry 图标按钮，右侧显示 `Discard audio`。retry 是唯一主恢复动作，discard 是退出恢复状态的轻量动作，二者不能重复成两个 retry 入口。任何临时语音状态都不能改变 send 的固定位置。
...
Session 列表：分成 Active 和 Archived 两段。Active 是默认工作集，Archived 是同屏折叠抽屉，不跳到另一个页面。本轮不放 session search，因为当前能力只是本地已加载 title filter，视觉上会误导成全量历史搜索。Active / Archived header 是 44pt 高的安静控制行：Title Case、13pt medium、右侧轻量 chevron，不显示计数、不画装饰横线、不使用 uppercase tracking，避免在窄 sidebar 中折行。每行标题 + 灰色摘要预览 + 时间，选中行只用极淡蓝底 + 蓝左条，不显示 checkmark；子 session 用竖线 + 圆点连接器表达父子。Archived 行仍然可点击查看，但标题和时间降一级灰，不画蓝左条，避免和当前工作项混在一起。Swipe action 沿用当前实现的按钮语言：图标在上、文字在下。Delete 永远在 trailing，红色；Archive / Restore 永远在 leading，克制蓝灰色；所有 action 禁用 full swipe，不弹确认。
...
Host Profiles（多 OpenCode 环境）沿用原生 Form / sheet 语言，不做自定义卡片系统。这里最重要的不是视觉差异，而是降低网络配置的认知负担：用户选择的是 OpenCode host，transport 只是到达方式。

![iPhone — Host Profiles](design_images/host_profiles_list_v1.png)

Add Host 使用两段式入口：顶部先给 `Import Host Config`，下面才是手动表单。Direct / SSH Tunnel 用 segmented control 切换。SSH 模式下不让用户编辑 `127.0.0.1:4096`，只显示 `Managed by SSH tunnel`；需要管理员提供的字段集中在 `SSH Gateway` section；设备公钥是独立 section，必须有可点击的 `Copy This Device Public Key`，不能只写在说明文案里。

![iPhone — Add SSH Host](design_images/host_profiles_add_v2.png)

设计评审结论：第一版原型把 “copy public key” 写成说明但没有提供动作，导致 SSH onboarding 断链；`opencode` 行也缺少 `SSH Username` 标签。修订版把 Import 放到最上方，把 SSH 参数来源写清楚，并把本地 OpenCode URL 从可编辑字段降级为系统管理值。后续实现时还要处理 Dynamic Type：`LabeledContent` 在大字号下需要允许 value 换行或纵向堆叠，避免 `Assigned Remote Port` 与数值互相挤压。

Host Profiles 后续设计复核把重点从 onboarding 转到运维：导入后的 profile 必须能被检查、修正和诊断。截图 harness 生成的证据位于 `/tmp/opencode-host-profiles-screenshots/`，覆盖 Settings Current Host、Hosts list、Host Detail、Import prefill 和 SSH import 后表单状态。复核后保留的 UI 结论是：Settings 和 Host Detail 都要展示可执行诊断；Hosts row 点击进入详情而不是直接切换；没有有效连接历史时显示 `Never connected`；SSH Host Detail 必须露出 Gateway Host、SSH Port、SSH Username、Assigned Remote Port、本地 URL、`Use This Host`、`Copy Host Config JSON` 和 `Copy This Device Public Key`。

本轮截图还暴露两个设计细节。第一，raw Swift error 不能进入用户界面，Basic Auth 被拒绝应显示类似 `OpenCode rejected Basic Auth. Check username and password.` 的动作型文案。第二，窄屏下 SSH row 摘要可能截断，列表摘要只承担识别和进入详情的作用，完整字段以 Host Detail 为准。

Key technical decisions：Host 表示一个 OpenCode 环境，而不是 SSH host；transport 分为 `direct` 与 `sshTunnel`，Tailscale / VPN / LAN / HTTPS 都属于 direct；Settings 顶部从全局连接表单改成 Current Host；Add Host 优先支持不含 secret 的 Import Config，再保留手动添加；SSH private key 默认是 device-level，同一设备的多个 SSH hosts 复用同一把 public key；Basic Auth password 按 profile 存 Keychain，profile 里只保存 reference；TOFU trusted host 按 SSH gateway `host:port` 绑定；SSH 模式下 `127.0.0.1:4096` 是 app 管理的 runtime URL，UI 不让用户编辑；切换 host 时停止 SSE、断开旧 SSH tunnel、应用新 profile、清空 current session selection，并重新拉 health / projects / sessions；第一版实现采用 profile 作为持久化 source of truth，再展开写回现有 `serverURL`、Basic Auth、`SSHTunnelManager.config` runtime 字段，避免一次性重构 APIClient / SSE / SSH tunnel。

### 大屏（iPad / visionOS，三栏）

同一套 token 与品牌，布局舒展：左 Session 列表、中 Chat（正文限宽居中，editorial 留白）、右文件/代码预览。代码预览的语法高亮要收敛到冷静灰阶为主，避免彩虹色。iPad sidebar 中 Files 保持默认收起；Session 区域直接显示独立折叠的 Active 和 Archived 分区，不放 search。Archive / Restore / Delete 的 swipe 方向与 iPhone 完全一致。
