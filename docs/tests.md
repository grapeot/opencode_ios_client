...
`ui_driver` v1 支持 `devices`、`launch`、`screenshot`、`tree`、`run-xcuitest`。其中 `tree` 是诚实的 screenshot-only observation：返回 `observability: screenshot_only`、空 `nodes` / `compact`，并带 warning。它不伪装完整 accessibility tree。需要精确 element 断言时，用 `run-xcuitest` 调用 focused UI harness；这比坐标点击或伪造 tree 更符合 iOS best practice。Tier 4 v1 用截图、XCTest summary 和 agent 判断组合出 verdict，无法判断时报告 BLOCKED。

这个结构保留 Android 已证明的原则：CLI 是 skeleton，agent 是判断层，test 是 prompt。不同点在于 iOS 没有 Android `uiautomator dump` 的等价低成本接口，所以 v1 用 XCTest 作为精确 UI 身份桥。等后续补到可靠 iOS accessibility tree 后，可以把 `configure-server`、`send-prompt`、`tap-label` 等高层命令加入 `ui_driver`。
