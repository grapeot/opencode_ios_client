...
当前最小落地是 `ReadToolCardIntegrationTests.swift`。它默认不运行 live path；只有设置 `OPENCODE_INTEGRATION_TESTS=1` 才会连真实 server。
...
`ui_driver` v1 支持 `devices`、`launch`、`screenshot`、`tree`。其中 `tree` 是诚实的 screenshot-only observation：返回 `observability: screenshot_only`、空 `nodes` / `compact`，并带 warning。它不伪装完整 accessibility tree。需要精确 element 断言时，用 Tier 2 XCUITest 或 Tier 3 integration；Tier 4 v1 用截图和测试输出做判断，无法判断时报告 BLOCKED。

这个结构保留 Android 已证明的原则：CLI 是 skeleton，agent 是判断层，test 是 prompt。等后续补到可靠 iOS accessibility tree 后，可以把 `configure-server`、`send-prompt`、`tap-label` 等高层命令加入 `ui_driver`。
