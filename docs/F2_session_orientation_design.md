...
全部沿用现有 Quiet Tech 系统，只新增一个青色 token。直接用到的：主色 `#3B82F6`（`DesignColors.Brand.primary`，交互/品牌）给 Needs you 和 Done·unread，青色 `#2DB7A3`（`DesignColors.Brand.teal`，新增）给 Running。金色 `#D9A621` 在系统里专属 chat 内的 AI working pulse，本方案不动它。Running 改用青色是因为它和主蓝同属冷色、过渡更和谐，又和 Needs you 的主蓝拉开色相。文字三级明暗阶梯现成：`Neutral.text`（primary）、`Neutral.textSecondary`（secondary）、`Neutral.textTertiary`（`DesignTokens.swift:44-51`），整行明暗分级直接映射到这三级，不用新增颜色。行内边距 `DesignSpacing.sm`（8pt）纵向、`DesignSpacing.lg`（16pt）横向，圆角 `DesignCorners.medium`（12pt）。现有左侧 3pt 选中竖条（`SessionListView.swift:195-206`）扩展成承载状态的色条。
...
| **Running** | 青色实心，3pt | 青色 `circle` + pulse 动画 | headline，`Neutral.text` | "运行中" + 已运行时长（live 计时） |
...
几个要点。Needs you 的强度来自四个维度同时拉满：最宽的主色条、实心图标、加粗满亮标题、一句带动词的文案，所以即使它排在第十行，扫过去也是整列里最跳的一个。Running 用青色而非主色，是因为青色和主蓝同属冷色、放一起是邻近色而非对比色，过渡自然，同时又和 Needs you 的主蓝拉开色相，两个强信号并置时不会糊成一团。金色留给 chat 内的 AI working pulse，列表里不出现。明暗分级是承载时间纵深的主力：Done·unread 满亮、Done·read 退到 secondary、Stale 退到 tertiary，越旧越淡，这条明暗梯度让陈旧会话自然沉下去，不需要折叠也不会干扰。
...
┃ 给搜索加分页            ⟳ 运行中 · 3m         ← Running：3pt 青色条 + pulse + live 计时
...
第二，Running 的青色 pulse 强度。青色和主蓝同属冷色，并置和谐，但它会和 Needs you 的主蓝在同一屏出现。两个强信号并置的视觉平衡、以及 pulse 的呼吸幅度，需要你在 hi-fi 里定。
...
## 六、4a 已实现（首版，待 hi-fi 微调）

4a 已经按上面的单行编码落地到代码，纯客户端改动，跑通了真机 simulator。这一版的目的是先把状态模型和排序验证出来，视觉强度等你在 hi-fi 里定稿后再微调。

### 真机截图

下图是 simulator 连到本地 4096 服务器、显示真实 session list 的截图。可以看到意图摘要标题替代了时间戳、未读会话带蓝色未读点和左侧状态条、标题按状态分明暗、排序仍是严格时间序。

![F2 4a session list 首版实现](design_images/f2_session_list_4a_temp.png)

这张是临时图，后续会删。截图里全部会话都是未读态（Done·unread），因为是全新安装、还没有 last-viewed 记录，所以未读底座正好在这里得到验证。Needs you 和 Running 当前这批数据里没有同时出现，等有阻塞/运行中的会话时会呈现主蓝条和青色 pulse。

### 这一版做了什么

- 新增青色 token `DesignColors.Brand.teal`（`#2DB7A3`），金色保留不动。
- 新增 `Models/SessionDisplayState.swift`：五状态枚举 + 纯函数 `derive(...)`，按 Needs you > Running > Done·unread > Stale > Done·read 的优先级从 session 数据推导，`now` 注入、可测试。
- blocked 信号纯客户端推导：`AppState.isSessionBlocked(_:)` 扫 `pendingPermissions` / `pendingQuestions` 的 `sessionID`，`sessionBlockKind(_:)` 区分"等你授权"和"等你回答"。
- read/unread 底座：`AppState+LastViewed.swift` 按现有 `AppState+Drafts` 的 UserDefaults 模式存 per-session last-viewed，`selectSession` 里打时间戳。
- 标题意图摘要：`Models/SessionTitleSummary.swift`，自动生成的时间戳标题回退到首条用户消息截断，真实标题保留，纯展示、不改服务端。
- `SessionRowView` 单行编码：左侧状态色条（4/3/2pt）、状态图标、标题字重与三级明暗、Running 的 `TimelineView` live 计时和青色 pulse、Done·unread 未读点。排序、折叠树、滑动删除、选中态都没动。
- 为五个状态加了 accessibility 标识（`session-state-<id>-<stateName>` + 行的 accessibilityValue），方便 UI 测试断言。

### 测试

- 新增 `OpenCodeClientTests/SessionDisplayStateTests.swift`，覆盖五状态推导（含优先级和边界）和标题摘要逻辑，20 个用例全过。
- 全量单元测试 274 个用例、45 个 suite 全过，build 在 iPhone 17 Pro / iOS 26 上成功。

### 待 hi-fi 定稿后再调的点

色条宽度配比、Running pulse 呼吸幅度、Stale 明暗够不够沉、结果摘要要不要默认开，这些都按第五章交还的问题等你在 hi-fi 里定。

---

## 七、第二轮重构（WIP，有已知 bug）

首版真机跑起来后发现一个信息架构问题：左侧把三个正交维度挤在同一条窄缝里。状态色条、未读蓝点、展开箭头三样东西都堆在标题左边几个像素内，且状态和未读都用主蓝，视觉上冗余且打架。更糟的是全新安装时满屏都是未读态，本该突出少数派的强信号变成了背景噪音。

这一轮按"三个维度三种不同手法"重构，把正交维度分开承载：

- 层级（树）用几何位置：展开箭头/缩进单独成最左一列，固定宽度。
- 状态用颜色（色条），且只给少数派：色条只在 needsYou（主蓝）和 running（青色）时出现，done/stale 不给色条。色条从"几乎每行都有"变回"只有需要注意的才有"。
- 未读用字重，不给独立色块：删掉蓝色未读点，未读靠标题加粗加上已读降一档明度表达。满屏未读时呈现的是一片有轻重的标题，而非一片闪烁蓝点。

改动落在 `SessionListView.swift` 的 `statusBar`（去掉 doneUnread 的条）、`stateIcon`（去掉 doneUnread 的点）、`titleFont`（未读用 headline、已读用 body）、以及 row body 改成两固定列布局（列1 层级、列2 状态图标占固定宽度让标题对齐）。全量测试 274 个仍全过。

### 已知 bug（真机截图暴露，尚未修）

这一轮重构引入了两个还没解决的视觉冲突，记在这里：

- 选中态蓝条和状态青条并排。选中态本身有一根左侧 3pt 蓝条（`selectionBackground`），状态色条又是独立 overlay 也挂在最左。一行同时"选中加 running"时，左侧出现蓝条加青条两根并排竖条，读起来很怪。
- running 状态被表达两次。running 行既有青色状态条，又有一个青色 pulse 圆点，同一状态用了色条和图标两种手法，违背"一个维度一种手法"。

待定的修法方向（未与维护者敲定，下一轮处理）：最左永远只留一列状态条，选中态改用圆角蓝底加右侧蓝勾表达、不画左条；running 去掉独立圆点，状态完全交给色条。Running pulse 放色条上还是改静止条，待定。

### per-device 未读机制（确认，非 bug）

未读判定是两个时间戳比较：服务端的 `session.time.updated` 和本地的 `lastViewedAt`。`lastViewedAt` 只在 `selectSession`（用户在本机点开会话）那一刻盖一次本地时间戳，存在本设备 UserDefaults，per-device、不跨设备同步。所以两台设备并行工作时，A 设备读过的会话在 B 设备上照样显示未读，这是当前设计的局限。按"工程怎么简单怎么做"的取向，维持 per-device 本地，跨设备同步（需服务端存 last-viewed）作为已知局限留待以后单独评估。

代码层面已确认 `markSessionViewed` 只有 `selectSession` 一个写入路径，没有被列表刷新或 SSE 误触发，所以已读判定本身没有 bug。一个正在 emit 新信息的会话只要本机没点开就应显示未读；若观察到没变未读，最可能是列表 `updated` 还没经 SSE 实时刷新（数据滞后），而非判定错。

### 临时截图状态

`docs/design_images/f2_session_list_4a_temp.png` 是首版（第二轮重构前）的 simulator 截图，反映的是旧的三色块拥挤布局，不代表当前代码。第二轮重构后的真机效果已在与维护者的对话里确认（running 青条加字重未读加干净留白方向对，但暴露了上面两个 bug）。这张临时图与所有 `docs/design_images/f2_*` 临时资源会在 PR merge 前删除，重要信息已并入本文档。
