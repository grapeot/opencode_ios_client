...
struct SessionMergePreserveCurrentTests {
    private func makeSession(id: String, directory: String = "/tmp") -> Session {
        Session(
            id: id,
            slug: id,
            projectID: "p1",
            ...
            title: "Title",
            version: "1",
            time: .init(created: 0, updated: 0, archived: nil),
            share: nil,
            summary: nil
        )
    }

    @Test func mergePrependsCurrentWhenMissingFromLoaded() {
        let s1 = makeSession(id: "s1", directory: "/projA")
        let s2 = makeSession(id: "s2", directory: "/projA")
        let current = makeSession(id: "current", directory: "/projB")
        let loaded = [s1, s2]

        let result = AppState.mergeCurrentSessionIfMissing(
            loaded: loaded,
            currentSessionID: "current",
            fetchedCurrent: current
        )

        #expect(result.count == 3)
        #expect(result[0].id == "current")
        #expect(result[1].id == "s1")
        #expect(result[2].id == "s2")
    }

    @Test func mergeReturnsLoadedWhenCurrentInList() {
    ...
    @Test func mergeReturnsLoadedWhenFetchedNil() {
        let loaded = [makeSession(id: "s1")]
    ...
    @Test func mergeReturnsLoadedWhenCurrentSessionIDNil() {
        let loaded = [makeSession(id: "s1")]

        let result = AppState.mergeCurrentSessionIfMissing(
            loaded: loaded,
            currentSessionID: nil,
            fetchedCurrent: makeSession(id: "s1")
        )
